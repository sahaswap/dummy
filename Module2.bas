...
' own tasks changes. Optimised trades run time for a much less
' bursty request pattern; Fast keeps a modest widen over the old
' flat v3.4 default (3-10s) instead of leaving it unchanged, since
' even Fast shouldn't be firing back-to-back at the old cadence.
Private Const RANDOM_LAUNCH_DELAY_ENABLED As Boolean = True
Private Const FAST_LAUNCH_DELAY_MIN_SEC As Single = 5
Private Const FAST_LAUNCH_DELAY_MAX_SEC As Single = 15
...
'------------------------------------------------------------------
' Search mode selection (v3.5)
'------------------------------------------------------------------
' Two chained Yes/No/Cancel-style prompts stand in for a real
' 3-button chooser. A true custom button dialog would need a new
' UserForm built in the VBA IDE's form designer (its control layout
' lives in a binary blob, not something that can be pasted in as
' plain code), so this reuses the same TopMostMsgBox mechanism
' already used everywhere else in this module - zero new IDE design
' work, works the moment this code is pasted in.
'
...
Beep
Dim modeResp As Long
modeResp = TopMostMsgBox( _
"Choose a search mode for this run:" & vbCrLf & vbCrLf & _
"YES = Fast" & vbCrLf & _
"   Headless, quickest, standard CAPTCHA defenses." & vbCrLf & vbCrLf & _
"NO = choose between Optimised or Visible" & vbCrLf & _
"   Both trade speed for a lower CAPTCHA hit rate." & vbCrLf & vbCrLf & _
"CANCEL = stop, don't run any searches", _
"Search Mode", MB_YESNOCANCEL Or MB_ICONQUESTION Or MB_TOPMOST)

If modeResp = IDCANCEL Then
PromptForSearchMode = False
Exit Function
End If

If modeResp = IDYES Then
ApplySearchMode smFast
PromptForSearchMode = True
Exit Function
End If

Beep
Dim modeResp2 As Long
modeResp2 = TopMostMsgBox( _
"YES = Optimised" & vbCrLf & _
"   Still headless, but with much wider random gaps between " & _
"searches. Same CAPTCHA-triggering fingerprint as Fast, far less " & _
"bursty request pattern - this run will take noticeably longer." & _
vbCrLf & vbCrLf & _
"NO = Visible" & vbCrLf & _
"   Real, visible browser windows instead of headless Edge - no " & _
"automation fingerprint at all, the lowest CAPTCHA risk of the " & _
"three. Slowest option; keep your hands off the mouse and " & _
"keyboard while it runs, same as before.", _
"Search Mode", MB_YESNO Or MB_ICONQUESTION Or MB_TOPMOST)

If modeResp2 = IDYES Then
ApplySearchMode smOptimised
Else
ApplySearchMode smVisible
End If
PromptForSearchMode = True
End Function
